...
function _extractAttrValue(attrs, attrName) {
  const res = []
  const r = new RegExp('(?:^|\\s)' + attrName + '=([\'"])(?:\\1|(.*?[^\\\\])\\1)', 'img')
  let m
  do {
    m = r.exec(attrs)
    if (m && m[2]) {
...
exports.asyncloadcss = function (content, target, attrs, alternateSearchPath) {
  const id = _genId(target)
  const onload = _extractAttrValue(attrs, 'onload')
  const onerror = _extractAttrValue(attrs, 'onerror')
  return [
